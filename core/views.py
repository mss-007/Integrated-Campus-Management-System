...
def admin_dashboard(request):
    ...
    return render(request, 'admin_dashboard.html', {
        'total_students': Student.objects.count(), 'total_boys': Student.objects.filter(gender__iexact="male").count(),
        'total_girls': Student.objects.filter(gender__iexact="female").count(), 'total_departments': Student.objects.values('department').distinct().count(),
        'teaching_faculty': Faculty.objects.filter(staff_type__icontains="teach").count(), 'non_teaching': Faculty.objects.filter(staff_type__icontains="non").count()
    })
...
def faculty_dashboard(request):
    ...
    # 🔥 NEW: Fetch dynamic custom fields for this faculty member
    ensure_static_fields()
...
def student_detail(request, id):
    ...
    if not (is_admin or is_self or is_faculty): return HttpResponseForbidden()
    ...
    return render(request, 'student_detail.html', {'student': student, 'fields': fields, 'is_admin': is_admin, 'is_faculty': is_faculty})

# 🔥 HERE IS THE FUNCTION THAT WAS MISSING!
@login_required
def student_full_details(request, id):
    ...
    return render(request, 'student_full_details.html', {'student': student, 'labels': labels, 'extra_data': extra_data, 'is_faculty': is_faculty, 'is_admin': is_admin})
...
def student_list(request):
    ...
    return render(request, 'student_list.html', {'students': students, 'is_admin': request.user.is_superuser or request.user.groups.filter(name="Admin").exists()})
...
def faculty_edit(request, id):
    # 🔥 SECURITY LOCK: Only Admins can access the edit route now
...
def digilocker(request):
    ...
    # 🔥 Lock Faculty out of the DigiLocker Admin Hub and send them to their student list
    if hasattr(request.user, 'faculty'): return redirect('student_list')
...
def student_locker(request, id):
    ...
    if request.method == 'POST':
        if is_faculty: return HttpResponseForbidden()
        action = request.POST.get('action')
    ...
    return render(request, 'student_locker.html', {'student': student, 'locker_items': locker_items, 'is_faculty': is_faculty, 'is_admin': is_admin})
...
def export_excel(request):
    ...
    if request.method == "POST":
        ...
        # 🔥 SECURITY LOCK: Force faculty to ONLY export their department's students
        if is_faculty:
            data_type = 'student'
...
def programmes(request):
    ...
    # 🔥 FIXED: Changed 'icontains' to 'iexact' so Females aren't counted as Males!
    ...
    dept_data = [{'name': d['department'], 'total_students': d['total_students'], 'boys': d['boys'], 'girls': d['girls'], 'faculty': Faculty.objects.filter(department=d['department']).count()} for d in departments]
